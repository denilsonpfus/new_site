node {
    ...
    stage ('Launch docker containers') {
      ...
      // Lauch website container here
           sh "docker run -d -t --name new_site${env.BUILD_NUMBER} denilsonpfus/website:${env.BUILD_NUMBER}" 
      // Lauch Apache container here
           sh "docker run -d -t --name apache_server${env.BUILD_NUMBER} denilsonpfus/apache_server:${env.BUILD_NUMBER}"
